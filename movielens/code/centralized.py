...
def f_centralized(R, num_iteration, cardinality, num_gradient_estimation_samples):
    '''
    Performs a centralized continuous greedy algorithm to select a subset of movies based on ratings.
    ...
    for t in range(num_iteration):
        ...
        grad = np.zeros(num_movies, dtype=np.float64)
        ...
        top_k_indices = np.argsort(grad)[-cardinality:]  # Get the indices of the top k values

        v = np.zeros_like(grad, dtype=int)
...
for i in range(len(vector_k)):
    k = vector_k [i]
    value = f_centralized(R, num_iteration=10, cardinality = k, num_gradient_estimation_samples = 20)
    f_k[i] = value
